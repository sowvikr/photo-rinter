...
(async()=>{
 ...
 try {
  ...
  let requestedSegmentUrl = '';
  await p.route('**/segment**',r=>{requestedSegmentUrl=r.request().url();r.fulfill({contentType:'image/png',body:Buffer.from(fixtures.mask,'base64')});});
  ...
  await p.waitForFunction(()=>document.getElementById('background-status').textContent.includes('result ready'));
  assert.match(requestedSegmentUrl, /quality=portrait/);
